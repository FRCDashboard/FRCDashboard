let ipc = require('electron').ipcRenderer, keys = {}, connectionListeners = [], connected = false, globalListeners = [], keyListeners = {}, robotAddress = "127.0.0.1";
ipc.send('ready');
...
let NetworkTables = {
    ...
    putValue(key, value) {
        if (key in keys) {
            ipc.send('update', { key, val: value, valType: valType(value), id: keys[key].id, flags: keys[key].flags });
        }
        else {
            ipc.send('add', { key, val: value, valType: valType(value), flags: 0 });
        }
        return connected;
    },
    /**
