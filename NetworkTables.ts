...
type sendMesg = { key: string, val: any, valType: number, id?: number, flags: number }
let ipc = require('electron').ipcRenderer,
    keys: { [key: string]: { val: any, valType, id, flags, new: boolean } } = {},
...
let NetworkTables = {
     ...
     */
    addRobotConnectionListener(f: (connected: boolean) => any, immediateNotify?: boolean): void {
     ...
     */
    addGlobalListener(f: (key: string, value: any, isNew: boolean) => any, immediateNotify?: boolean): void {
     ...
     */
    addKeyListener(key: string, f: (key: string, value: any, isNew: boolean) => any, immediateNotify?: boolean): void {
    ...
    putValue(key: string, value: any): boolean {
        if (key in keys) {
            ipc.send('update', { key, val: value, valType: valType(value), id: keys[key].id, flags: keys[key].flags } as sendMesg)
        } else {
            ipc.send('add', { key, val: value, valType: valType(value), flags: 0 } as sendMesg)
        }
        return connected
    },
